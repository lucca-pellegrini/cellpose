...
from cellpose import denoise, dynamics, io, metrics, models, transforms, utils
...
def seg_eval_cp3(folder, noise_type="poisson"):
    ...
    for c, ctype in enumerate(ctypes):
        print(ctype)
        pretrained_models = [
            f"/home/carsen/.cellpose/models/{model_names[noise_type]}{istr}_{ctype}"
            for istr in ["_rec", "_seg", "_per", ""]
        ]
        pretrained_models.extend(
            [
                f"/home/carsen/.cellpose/models/{model_names[noise_type]}_cyto3",
        ...
        diam_test = (
            dat["diam_test"][:nimg_test]
            if "diam_test" in dat
            else diam_mean * np.ones(len(test_noisy))
        )
        ...
        for istr, pretrained_model in zip(istrs, pretrained_models):
            ...
            imgs2 = dn_model.eval(
                [test_noisy[i][0] for i in range(len(test_noisy))],
                diameter=diam_test,
                channel_axis=0,
            )

            masks2, flows, styles2 = seg_model.eval(
                imgs2,
                channels=[1, 0],
                diameter=diam_test,
                channel_axis=-1,
        ...
        if noise_type == "poisson":
            ### cellpose retrained
            dat = np.load(
                root / "noisy_test" / f"test_{noise_type}.npy",
                allow_pickle=True,
            ).item()
            ...
            diam_test = (
                dat["diam_test"]
                if "diam_test" in dat
                else 30.0 * np.ones(len(test_noisy))
            )

            seg_model = models.CellposeModel(
                gpu=True, nchan=1, model_type=f"{ctype}_noisy"
            )
            masks2 = seg_model.eval(
                test_noisy, channels=None, diameter=diam_test, normalize=False
            )[0]
...
def blind_denoising(folder):
    ...
    for ctype in ctypes:
        ...
        dat = np.load(
            root / "noisy_test" / f"test_poisson.npy", allow_pickle=True
        ).item()
        masks_true = dat["masks_true"]
...
def real_examples(folder):
    ...
    for dset in dsets:
        ...
        if dset == "Projection_Flywing":
            ...
            clean = [
                transforms.normalize99(
                    img[-1][img.shape[1] // 2 - dz : img.shape[1] // 2 + dz].max(axis=0)
                )
                for img in imgs
            ]
            diam = 12.0
        ...
        model = denoise.DenoiseModel(
            gpu=True, nchan=1, diam_mean=diam_mean, model_type=model_type
        )
        seg_model = models.CellposeModel(
            gpu=True, model_type=cp_model_type, diam_mean=diam_mean
        )
        masks_clean = seg_model.eval(
            clean,
            diameter=diam,
        ...
        for nl in range(3):
            if dset == "Projection_Flywing":
                ...
                noisy = [
                    transforms.normalize99(
                        img[nl][img.shape[1] // 2 - dz : img.shape[1] // 2 + dz].max(
                            axis=0
                        )
                    )
                    for img in imgs
                ...
                noisy = [
                    transforms.normalize99(
                        img[img.shape[0] // 4 : img.shape[0] // 4 + dz].max(axis=0)
                    )
                    for img in noisy
                ]
            ...
            thresholds = np.arange(0.5, 1.05, 0.05)
            ap_noisy = metrics.average_precision(
                masks_clean, masks_noisy, threshold=thresholds
            )[0]
            ap_denoised = metrics.average_precision(
                masks_clean, masks_denoised, threshold=thresholds
            )[0]
        ...
        for i in range(len(noisy)):
            pred_val = noise2self.train_per_image(
                noisy[i][np.newaxis, ...].astype("float32")
            )
...
def real_examples_ribo(root):
    ...
    masks = seg_model.eval(
        clean, diameter=diameter, channels=[0, 0], normalize=normalize
    )[0]
    ...
    for n, imgs in enumerate(noisy):
        masks_noisy = seg_model.eval(
            imgs, diameter=diameter, channels=[0, 0], normalize=normalize
        )[0]
        img_dn = model.eval(
            imgs, diameter=diameter, channels=[0, 0], normalize=normalize
        )
        ap, tp, fp, fn = metrics.average_precision(
            masks, masks_noisy, threshold=thresholds
        )
        ap_noisy[n] = ap
        masks_dn = seg_model.eval(
            img_dn, diameter=diameter, channels=[0, 0], normalize=normalize
        )[0]
        ap, tp, fp, fn = metrics.average_precision(
            masks, masks_dn, threshold=thresholds
        )
        ap_dn[n] = ap
    ...
    for n, imgs in enumerate(noisy):
        ...
        ap, tp, fp, fn = metrics.average_precision(
            masks, masks_n2s, threshold=thresholds
        )
        dat["ap_n2s"][n] = ap
    ...
    for n, imgs in enumerate(noisy):
        ...
        masks_n2v = seg_model.eval(
            imgs_n2v, diameter=diameter, channels=[0, 0], normalize=normalize
        )[0]
        ap, tp, fp, fn = metrics.average_precision(
            masks, masks_n2v, threshold=thresholds
        )
        # print(ap[:,0])
        dat["ap_n2v"][n] = ap
...
def specialist_training(root):
    ...
    imgs, masks_n2s, ap = noise2self.train_test_specialist(
        root, n_epochs=50, lr=5e-4, test=True
    )
