    ...
    for mtype in ["default", "transformer"]:
        if mtype == "default":
            model = models.Cellpose(
                gpu=True, nchan=3, model_type="neurips_cellpose_default"
            )
            channels = None
            normalize = False
            ...
            model = models.CellposeModel(
                gpu=True,
                nchan=3,
                model_type="neurips_cellpose_transformer",
            ...
            diams = dat[
                "diams_pred"
            ]  # (use diams from Cellpose default model for transformer)
...
def prediction_tuning(root, root2=None):
    ...
    for mtype in model_types[:-1]:
        ...
        if mtype == "grayscale" or mtype == "maetal":
            if mtype == "grayscale":
                model = models.CellposeModel(
                    gpu=True, model_type="neurips_grayscale_cyto2"
                )
            else:
                ...
                if not os.path.exists(pretrained_model):
                    print(
                        "need to download cellpose model from Ma et al; https://github.com/JunMa11/NeurIPS-CellSeg/tree/main/cellpose-omnipose-KIT-GE"
                    )
        ...
        out = model.eval(
            imgs if mtype == "grayscale" else imgs_norm,
            diameter=diams,
            channels=channels,
            normalize=normalize,
            tile_overlap=0.6,
    ...
    if root2 is not None:
        ...
        for imgf in img_files:
            maskf = root2 / (
                os.path.splitext(os.path.split(imgf)[-1])[0] + "_label.tiff"
            )
            m = io.imread(maskf)
    ...
    for mtype in model_types:
        ...
        ap, tp, fp, fn = metrics.average_precision(
            masks_true, masks_pred, threshold=thresholds
        )
        f1 = 2 * tp / (2 * tp + fp + fn)
