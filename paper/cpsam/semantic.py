...
from cellpose import dynamics, io, metrics, models, train, transforms, vit_sam
...
def convert_monusac_train_data(root):
    img_files = [
        f
        for folder in natsorted(root.glob("*"))
        for f in natsorted(folder.glob("*.tif"))
    ...
    for i, (xml_file, img_file) in enumerate(zip(lbl_files, img_files)):
            ...
            for child in root[k]:
                for x in child:
                    ...
                    if r == "Region":
                        ...
                        gt = (
                            gt + 1
                        )  # Keep track of giving unique valu to each instance in an image
        ...
        for j in range(ny):
            ...
            for k in range(nx):
                ...
                if (masks0 > 0).sum() > 0:
                    img0 = transforms.normalize_img(img0, axis=-1).transpose(2, 0, 1)[
                        :3
                    ]
                    lbl_all = dynamics.labels_to_flows(
                        [masks0], device=torch.device("cuda")
                    )[0]
                    ...
                    lbls = np.concatenate(
                        (lbl_all[:1], cp_all[np.newaxis, ...], lbl_all[1:]),
                        axis=0,
                    )

                    tifffile.imwrite(
                        root0 / "train" / f"monusac_{i}_{j}_{k}.tif",
                        data=img0,
                        compression="zlib",
                    )
                    tifffile.imwrite(
                        root0 / "train" / f"monusac_{i}_{j}_{k}_flows.tif",
                        data=lbls,
                        compression="zlib",
                    )
                    tifffile.imwrite(
                        root0 / "train" / f"monusac_{i}_{j}_{k}_masks.tif",
                        data=fastremap.renumber(masks0.astype("uint16"))[0],
                        compression="zlib",
                    )
...
def convert_monusac_test_data(root):
    ...
    for i, (xml_file, img_file) in tqdm(enumerate(zip(lbl_files, img_files))):
        ...
        tifffile.imwrite(
            root0 / f"{img_file.stem}_classes.tif",
            data=cp_all,
            compression="zlib",
        )
        tifffile.imwrite(
            root0 / f"{img_file.stem}_masks.tif",
            data=masks.astype("uint16"),
            compression="zlib",
        )
        tifffile.imwrite(
            root0 / f"{img_file.stem}_masks_bad.tif",
            data=ibad.astype("uint16"),
            compression="zlib",
        )
...
def initialize_class_net(nclasses=5, device=torch.device("cuda")):
    ...
    # default model
    net.load_model(
        "models/cpsam8_0_2100_8_402175188",
        device=device,
        strict=False,
        multigpu=False,
    )
...
def train_net(root0):
    ...
    train_labels = [
        io.imread(str(train_files[i])[:-4] + f"_flows.tif")
        for i in trange(len(train_files))
    ]
    train_masks = [
        io.imread(str(train_files[i])[:-4] + f"_masks.tif")
        for i in trange(len(train_files))
    ]
...
def test_net(root0):
    ...
    net.load_model(
        "models/he_monusac_pclass_final2_batch_size_16",
        device=torch.device("cuda"),
        strict=False,
        multigpu=False,
    )
    ...
    masks_bad = [
        io.imread(str(f).replace(".tif", "_masks_bad.tif")).squeeze() for f in img_files
    ]
    classes_true = [
    ...
    aps_img, errors_img = compute_ap_pq(
        masks_true, masks_bad, classes_true, masks_pred, classes_pred
    )
    ...
    np.save(
        "results/monusac_cellposeSAM.npy",
        {
            "errors": errors_img,
            "aps": aps_img,
...
def compute_leader_scores(root0):
    ...
    for lfolder in leader_folders:
        ...
        for folder in folders:
            ...
            iouc, tpc, fpc, fnc = (
                np.zeros((len(ids), 4), "float32"),
                np.zeros((len(ids), 4), "int"),
                np.zeros((len(ids), 4), "int"),
                np.zeros((len(ids), 4), "int"),
            )
            for i in ids:
                rgb_file = Path(
                    f"/media/carsen/ssd3/datasets_cellpose/images_HandE/MoNuSAC/{lfolder}/{folder}/{img_files[i].stem}_mask.png.tif"
                )
        ...
        aps_img, errors_img = compute_ap_pq(
            masks_true, masks_bad, classes_true, masks_pred_l, classes_pred_l
        )
        print(np.nanmean(errors_img, axis=0), np.nanmean(errors_img))
        print(np.nanmean(aps_img, axis=0), np.nanmean(aps_img))

        np.save(
            f"results/monusac_{lfolder}.npy",
            {
                "errors": errors_img,
                "aps": aps_img,
