...
@pytest.mark.parametrize(
    ...
    [  # passing:
        ...
        (
            ...
            (100, 120, 3),
            False,
        ),  # 2D RGB image with channels first
        (
            (3, 100, 120),
            None,
            None,
    ...
    if raises_error:
        with pytest.raises(ValueError):
            transforms.convert_image(
                img, channel_axis=channel_axis, z_axis=z_axis, do_3D=do_3D
            )
    else:
        ...
        assert (
            converted_img.shape == expected_shape
        ), f"Expected shape {expected_shape}, but got {converted_img.shape}"
