...
def eval_3D(root, ntrain, transformer, seed):
    ...
    if transformer:
        ...
        if ntrain == -1:
            ...
            model = models.CellposeModel(
                gpu=True, pretrained_model=str(root / f"models/{netstr}")
            )
    ...
    for i, f in enumerate(test_files):
        ...
        if transformer:
            masks, flows, styles = model.eval(
                np.stack((img, 0 * img, 0 * img), axis=-1),
                channel_axis=-1,
                z_axis=0,
                niter=1000,
                flow3D_smooth=2,
        ...
        if seed == 0:
            io.imsave(
                root / f"test_3D/{f.stem}_ntrain_{ntrain}_{mstr}_masks.tif",
                masks,
            )
    ...
    np.save(
        root / f"models/{netstr}_3D_AP_TP_FP_FN.npy",
        {
            "threshold": threshold,
            "ap": ap,
            "tp": tp,
