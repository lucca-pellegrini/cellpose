...
class CellposeModel:
    ...
    def _resize_cellprob(
        ...
        if prob.ndim == 2:
            # 2D case:
            prob = transforms.resize_image(
                prob, Ly=to_y_size, Lx=to_x_size, no_channels=True
            )
            if squeeze_happened:
                prob = np.expand_dims(
                    prob, int(np.argwhere(prob_shape == 1))
            ...
            prob = transforms.resize_image(
                prob, Ly=to_z_size, Lx=to_x_size, no_channels=True
            )
            prob = prob.transpose(1, 0, 2)
        else:
            raise ValueError(
    ...
    def _resize_gradients(
        ...
        if grads.ndim == 3:
            ...
            grads = transforms.resize_image(
                grads, Ly=to_y_size, Lx=to_x_size, no_channels=False
            )
            grads = np.moveaxis(grads, -1, 0)  # Put gradients first

            if squeeze_happened:
                grads = np.expand_dims(
                    grads, int(np.argwhere(grads_shape == 1))
                )  # add back empty axis for compatibility
        elif grads.ndim == 4:
            ...
            grads = transforms.resize_image(
                grads, Ly=to_z_size, Lx=to_x_size, no_channels=False
            )
