...
from cellpose import io, metrics, models, transforms
...
def load_dataset(dset):
    if dset != "monuseg":
        files = [
            f
            for f in Path(
                f"/media/carsen/ssd3/datasets_cellpose/images_{dset}/test/"
            ).glob("*.tif")
        ]
        files = natsorted(
            [f for f in files if "_masks" not in str(f) and "_flows" not in str(f)]
        )
        masks_true = [
            io.imread(str(f).replace(".tif", "_flows.tif"))[0].astype("uint16")
            for f in files
        ]
    else:
        files = [
            f
            for f in Path(
                f"/media/carsen/ssd3/datasets_cellpose/images_HandE/MoNuSeg/MoNuSegTestData/"
            ).glob("*.tif")
        ]
        files = natsorted([f for f in files if "_masks" not in str(f)])
        masks_true = [
            io.imread(str(f).replace(".tif", "_masks.tif")).astype("uint16")
            for f in files
        ]
    ...
    for img in imgs0:
        if img.ndim == 2:
            ...
            if img.shape[0] < 3:
                img = np.concatenate(
                    (
                        np.zeros((3 - img.shape[0], *img.shape[1:]), dtype=img.dtype),
                        img,
                    ),
                    axis=0,
                )
...
def run_cellposesam(dsets=None):
    ...
    for dset in dsets:
        ...
        for i in trange(len(imgs)):
            img = imgs[i]
            tic = time.time()
            masks_pred0, flows, styles = model.eval(
                img,
                augment=False,
        ...
        ap, tp, fp, fn = metrics.average_precision(
            masks_true, masks_pred, threshold=threshold
        )
        print(ap.mean(axis=0)[[0, 5, 8]])
        print(((fp + fn) / (fn + tp)).mean(axis=0)[[0, 5, 8]])

        np.save(
            f"results/cellposesam_{dset}.npy",
            {
                "ap": ap,
                "tp": tp,
                "fp": fp,
    ...
    for img in imgs0:
        if img.ndim == 2:
            ...
            if img.shape[0] < 3:
                ...
                img = np.concatenate(
                    (
                        np.zeros((2, *img.shape[1:]), "float32"),
                        img.astype("float32").mean(axis=0, keepdims=True),
                    ),
                    axis=0,
                )
        img = img.astype(np.float32)
        ...
        if Lyr < 512:
            padyx[0] = [
                int(np.floor((512 - Lyr) / 2)),
                int(np.ceil((512 - Lyr) / 2)),
            ]
        if Lxr < 512:
            padyx[1] = [
                int(np.floor((512 - Lxr) / 2)),
    ...
    for dset in dsets:
        ...
        for i in trange(len(imgs)):
            ...
            if padyx[0][0] > 0 or padyx[1][0] > 0 or padyx[0][1] > 0 or padyx[1][1] > 0:
                masks = masks[
                    padyx[0][0] : masks.shape[0] - padyx[0][1],
                    padyx[1][0] : masks.shape[1] - padyx[1][1],
                ]
            Ly, Lx = (
                imgs[i].shape[-2:]
                if np.array(imgs[i].shape).argmin() == 0
                else imgs[i].shape[:2]
            )
...
def run_samcell(dsets=None):
    ...
    for dset in dsets:
        ...
        imgsCS, padCS = convert_images_samcell(
            imgs, resize=True if dset == "cyto2" else False
        )
        masks_pred = []
        runtime = []
        for i in trange(len(imgs)):
        ...
        threshold = np.arange(0.5, 1.0, 0.05)
        ap, tp, fp, fn = metrics.average_precision(
            masks_true[: len(masks_pred)], masks_pred, threshold=threshold
        )
        print(ap.mean(axis=0)[[0, 5, 8]])
        print(((fp + fn) / (fn + tp)).mean(axis=0)[[0, 5, 8]])

        np.save(
            f"results/samcell_{dset}.npy",
            {
                "ap": ap,
                "tp": tp,
                "fp": fp,
...
def run_cyto3_segformer(dsets=None):
    ...
    for dset in dsets:
        ...
        for i in trange(len(imgs)):
            ...
            masks_pred0, flows, styles = model.cp.eval(
                img,
                diameter=diam,
                channels=[1, 2],
                niter=2000 if "bac" in dset else None,
        ...
        model = models.CellposeModel(
            gpu=True, backbone="transformer", model_type="transformer_cp3"
        )
        masks_pred = []
        runtime = []
        for i in trange(len(imgs)):
...
def run_pathosam():
    ...
    for i in trange(len(imgs)):
        tic = time.time()
        masks_pred0 = automatic_segmentation.automatic_segmentation_wsi(
            imgs[i],
    ...
    np.save(
        f"results/pathosam_monuseg.npy",
        {
            "ap": ap,
            "tp": tp,
            "fp": fp,
