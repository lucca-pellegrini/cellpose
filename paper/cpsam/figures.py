...
from cellpose import denoise, io, metrics, transforms, utils
...
def fig2(save_fig=False):
    ...
    colors = [
        "g",
        np.maximum(0, np.array(colors_tab[1]) - 0.1),  # [0,1,0],
        [0.5, 0.3, 0],
        [0.7, 0.4, 1],
    ]  # [0.8,0.8,.3]]
    ...
    if 1:
        ...
        for i, tname in enumerate(
            ...
            ax.set_position(
                [
                    pos[0] + (len(type_names) - i - 1) * 0.012 - (i == 0) * 0.02 - 0.05,
                    pos[1] - 0.2 * pos[2],
                    pos[2] * 0.95,
                    pos[2] * 2,
                ]
            ...
            if i == 0:
                ...
                ax.text(
                    -0.1,
                    1.25,
                    "Style vector correlation between train and test images",
                    fontsize="large",
                ...
                transl = mtransforms.ScaledTranslation(
                    -20 / 72, 25 / 72, fig.dpi_scale_trans
                )
                il = plot_label(ltr, il, ax, transl, fs_title)

            ax.plot(
                -0.05 * len(styles_t) * np.ones(2),
                [len(styles), len(styles) - 100],
                color="k",
            )
        ...
        for i in range(len(type_names)):
            ...
            ax.plot(
                np.array([-1, 1]) * 0.3 + i,
                cc_stats[i].mean(axis=0) * np.ones(2),
                color=0.5 * np.ones(3),
                lw=3,
            )
    ...
    try:
        ...
        masks_H2 = [
            np.load(
                f"/media/carsen/ssd3/datasets_cellpose/images_cyto2/labels2/{i:03d}_img_seg.npy",
                allow_pickle=True,
            ).item()["masks"]
            for i in ind_im
        ]
    ...
    for i in iplot_errors:
        ...
        ax.plot(
            i + 2 + 0.3 * np.array([-1, 1]),
            errors[i, :, 0].mean() * np.ones(2),
            color=colors[i],
            lw=3,
        )
        if i < len(aps) - 1:
            ...
            axin.plot(
                [i + 2, len(aps) - 1 + 2],
                np.ones(2) * (0.95 + (len(aps) - 1 - i) * 0.02),
                lw=1,
                color="k",
            )
            axin.text(
                (len(aps) - 2 - i + len(aps) - 1) / 2 + 2,
                0.95 + (i + 1) * 0.02,
                "***",
                ha="center",
                va="center",
    ...
    for i in iplot_ia:
        ax.plot(
            [1.5, 6.5],
            errors_ia[i].mean() * np.ones(2),
            lw=2,
            color=colors_ia[i],
            ls="--" if i == 0 else "-",
        )
    ax.set_ylim([0, 0.8])
    ax.set_xlim([-0.75, (len(aps) + 1) + 0.75])
    axin.set_xlim([-0.75, (len(aps) + 1) + 0.75])
    axin.axis("off")
    ...
    ax.set_title(
        "Performance on Cellpose test set", fontstyle="italic", y=1.22, x=-0.08
    )
    ...
    vp = ax.violinplot(
        ap, positions=[-1], widths=0.6, showmeans=True, showextrema=False
    )
    ...
    ax.set_ylim([0.3, 1.0])
    ax.set_xlim([-1.5, len(aps) - 1 + 0.5])
    axin.set_xlim([-1.5, len(aps) - 1 + 0.5])
    axin.axis("off")
    ...
    for i in iplot_errors:
        ax.scatter(
            npix + np.random.rand(len(npix)) * npix * 0.05,
            runtimes[i],
            color=colors[i],
            s=10,
            lw=1.5,
    ...
    ax.xaxis.set_minor_locator(
        plt.FixedLocator([*np.arange(200, 1000, 100), *np.arange(1000, 3000, 1000)])
    )
    ...
    grid1 = matplotlib.gridspec.GridSpecFromSubplotSpec(
        2, 4, subplot_spec=grid[3:, :-3], wspace=0.0, hspace=0.1
    )
    ylims = [
        [70, 300],
        [0, 200],
    ...
    for i, iex in enumerate(
        [22, 19, 60, 15, 16, 20, 40, 47]
    ):  ##[56, 60, 53, 66]):#, 62]):
        ...
        img = (
            ...
            else np.concatenate((np.zeros_like(img[:1]), img), axis=0)
        )
        ...
        ax = plt.subplot(
            grid1[i // 4, i % 4]
        )  # grid[1+(i>1), 2*i+k -4*(i>1)])  #2,6,k+1 + 2*i)
        pos = ax.get_position().bounds
        ax.set_position(
            [
                pos[0] - 0.05 * pos[2] * (i % 4) - 0.01,
                pos[1] + ((i // 4) == 0) * 0.02 - 0.02,
                pos[2] * 1.0,
                pos[3] * 1.0,
            ]
        )
        ...
        for j, masks in enumerate([masks_preds[-1][iex]]):
            ...
            for outline in outlines:
                ax.plot(
                    ...
                    outline[:, 1],
                    color=outcols[j],
                    lw=1.5 if j == 0 else 2,
                    ls="dashed" if j == 0 else "-",
                )  # , dashes=[2, 3] if j!=0 else [])
        ax.axis("off")
        if i == 0:
            ax.set_title(
                "Example segmentations from Cellpose test set",
                fontstyle="italic",
                y=1.05,
                x=-0.0,
            )
            transl = mtransforms.ScaledTranslation(
                -14 / 72, 12 / 72, fig.dpi_scale_trans
            )
            il = plot_label(ltr, il, ax, transl, fs_title)
        elif i == 3:
...
def fig3(save_fig=False):
    ...
    colors = np.array(
        [
            ...
            [0, 1, 0],
            colors_tab[6],
            np.maximum(0, np.array(colors_tab[1]) - 0.1),
            [0.7, 0.4, 1],
        ]
    )
    ...
    grid1 = matplotlib.gridspec.GridSpecFromSubplotSpec(
        1, 4, subplot_spec=grid[0, :4], wspace=0.1, hspace=0.2
    )
    ...
    for i in range(3):
        sz = szs[i * 2]
        diam = diam_true[iex].copy() * 30.0 / sz
        ax = plt.subplot(grid1[0, i])
        pos = ax.get_position().bounds
        ax.set_position([pos[0] - 0.03 * i, pos[1], pos[2], pos[3]])
        img_rsz = transforms.resize_image(
            imgs[iex].copy().transpose(2, 1, 0), rsz=30.0 / diam
        )
        img_rsz = np.concatenate((np.zeros_like(img_rsz[:, :, :1]), img_rsz), axis=-1)
        masks_true_rsz = transforms.resize_image(
            masks_true[iex],
            rsz=30.0 / diam,
            no_channels=True,
    ...
    for j in range(3):
        ax.errorbar(
            np.arange(5),
            aps[j, :, :, 0].mean(axis=-1).T,
            aps[j, :, :, 0].std(axis=-1).T / (66**0.5),
    ...
    for ii, noise_type in enumerate(["poisson", "downsample", "blur", "aniso"]):
        ...
        grid1 = matplotlib.gridspec.GridSpecFromSubplotSpec(
            1,
            4,
            subplot_spec=grid[ii // 2 + 1, (ii % 2) * 4 : 4 + 4 * (ii % 2)],
            wspace=0.1,
            hspace=0.2,
        )
        ...
        for i in range(3):
            ...
            img = (
                denoise.add_noise(torch.from_numpy(img).unsqueeze(0), **params)
                .cpu()
                .numpy()
                .squeeze()
            )
            ...
            img_rsz = np.concatenate(
                (np.zeros_like(img_rsz[:, :, :1]), img_rsz), axis=-1
            )
            masks_true_rsz = masks_true[iex].copy()
            ...
            ax.imshow(
                np.clip(img_rsz[:, :, :] * vmax, 0, 1),
                aspect=1 if noise_type != "aniso" else param[k],
                interpolation="nearest",
            )  # .transpose(1,0,2))
            ycent, xcent = img_rsz.shape[0] // 2, img_rsz.shape[1] // 2
            ax.set_ylim([ycent - 65, ycent + 65])
            ...
            if i == 0:
                ax.set_title(
                    f"robustness to {nstr[ii]}", loc="left", fontstyle="italic"
                )
                il = plot_label(ltr, il, ax, transl, fs_title)
            ax.set_title(
                ["low", "medium", "high"][i],
        ...
        for j in range(4):
            ax.errorbar(
                ...
                aps[j, :, :, 0].mean(axis=-1).T,
                aps[j, :, :, 0].std(axis=-1).T / (66**0.5),
                color=colors[[4, 0, 0, 3]][j],
                ls="--" if j == 2 else "-",
            )
            if 1:
                xpos = 1.1 if j < 2 else 0.05
                ypos = (
                    1.05 - j * 0.15 + 0.1 * (ii == 3) if j < 2 else 0.3 - (j - 2) * 0.15
                )
                ax.text(
                    xpos,
                    ypos,
...
def fig4(save_fig=False):
    ...
    dset_names = [
        "BlastoSPIM (Nunley et al 2024)",
        "PlantSeg: lateral root (Wolny et al 2020)",
        "PlantSeg: ovules (Wolny et al 2020)",
    ]
    il = 0
    transl = mtransforms.ScaledTranslation(-15 / 72, 18 / 72, fig.dpi_scale_trans)
    grid1 = matplotlib.gridspec.GridSpecFromSubplotSpec(
        3, 2, subplot_spec=grid[:, :3], wspace=0.05, hspace=0.45
    )
    grid2 = [
        matplotlib.gridspec.GridSpecFromSubplotSpec(
            3, 1, subplot_spec=grid[:, 3:5], wspace=0.4, hspace=0.45
        ),
        matplotlib.gridspec.GridSpecFromSubplotSpec(
            3, 1, subplot_spec=grid[:, -2:], wspace=0.4, hspace=0.45
        ),
    ]
    grid3 = matplotlib.gridspec.GridSpecFromSubplotSpec(
        3, 2, subplot_spec=grid[:, -5:-2], wspace=0.4, hspace=0.45
    )
    for d, dset in enumerate(["blastospim", "root", "ovules"]):
        ...
        if dset == "root" or dset == "ovules":
            root = Path(
                f"/home/carsen/dm11_string/datasets_cellpose/root_ovules_wolny/{dset}/models/"
            )
        else:
        ...
        for i in range(2):
            ...
            ax.text(
                1,
                -0.1,
                f"AP@0.5={aps[1, itrain[i], 0, iex, 0]:.2f}",
                transform=ax.transAxes,
                ha="right",
            )
            ax.axis("off")
            if i == 0:
                ...
                il = plot_label(ltr, il, ax, transl, fs_title)
            ax.set_title(
                r"# of train ROIs = %d" % int(nrois[0, itrain[i]]),
                fontsize="medium",
            )
            # else:
        ...
        for k in range(2):
            ...
            ax.set_position(
                [
                    pos[0] + 0.2 * pos[2] + 0.14 * pos[2] * (k == 1),
                    pos[1] - 0.07 * pos[3],
                    pos[2] * 0.7,
                    pos[3] * 1.1,
                ]
            ...
            for j in range(2):
                ax.errorbar(
                    np.nanmean(nrois[:, 1:], axis=0) * frac,
                    np.nanmean(accs[j, 1:], axis=-1),
                    np.nanstd(accs[j, 1:], axis=-1) / (accs.shape[2] - 1) ** 0.5,
                    color=colors[j],
                )
                ax.errorbar(
                    1,
            ...
            if d != 2:
                ax.set_xticklabels(
                    [rf"10$^{t}$" if t != 0 else "0" for t in range(len(xticks))]
                )
            else:
                ax.set_xticklabels(
                    [rf"10$^{t + 1}$" if t != 0 else "0" for t in range(len(xticks))]
                )
            ...
            if d == 0:
                ax.xaxis.set_minor_locator(
                    plt.FixedLocator(
                        np.hstack([np.arange(1, 10) * j for j in [1, 10, 100, 1000]])
                    )
                )
                ax.text(
                    1.0,
                ...
                ax.xaxis.set_minor_locator(
                    plt.FixedLocator(
                        np.hstack(
                            [np.arange(1, 10) * j for j in [1, 10, 100, 1000, 10000]]
                        )
                    )
                )
        ...
        for j in range(2):
            ...
            ax.set_position(
                [
                    pos[0] + 0.07 * pos[2] - 0.08 * pos[2] * (j == 1),
                    pos[1] - 0.15 * pos[3],
                    pos[2] * 1.2,
                    pos[3] * 1.22,
                ]
            ...
            ax.imshow(im)
            ax.set_title(
                f"# of train ROIs = {int(nrois[0, itrain[j]])}",
                fontsize="medium",
                loc="left",
            )
            ax.text(
                1,
                -0.1,
                f"AP@0.5={aps[1, itrain[j], 0, iexs_3D[d], 0]:.2f}",
                transform=ax.transAxes,
                ha="right",
            )
            if d == 0 and j == 0:
...
def fig5(save_fig=False):
    ...
    for i, iex in enumerate(iexs):
        ...
        if i == 0:
            ax.text(
                0,
                3,
                "MoNuSAC 2020 challenge: segmentation and classification",
                fontsize="large",
                fontstyle="italic",
                transform=ax.transAxes,
            )
            ...
            for k, cname in enumerate(cl_names):
                ax.text(
                    ...
                    2.2 - k * 0.3,
                    cname,
                    color=class_colors_true[k],
                    transform=ax.transAxes,
                )
            transl = mtransforms.ScaledTranslation(
                -0 / 72, 55 / 72, fig.dpi_scale_trans
            )
            il = plot_label(ltr, il, ax, transl, fs_title)
    ...
    for k in range(2):
        ...
        for j in range(1, 5):
            ...
            p = wilcoxon(
                np.nanmean(eps[0], axis=-1), np.nanmean(eps[j], axis=-1)
            ).pvalue
            print(k, j, p)
            pstr = (
                "n.s."
                if p > 0.05
                else ("*" if p >= 0.01 else "**" if p >= 0.001 else "***")
            )
            axin.plot(
                [0, j],
                np.ones(2) * (0.95 + (len(eps) - j) * 0.02),
                lw=1,
                color="k",
            )
            pstr = (
            ...
            axin.text(
                j / 2,
                0.95 + (len(eps) - j) * 0.02 + 0.01 * (p > 0.05),
                pstr,
                ha="center",
                va="center",
                fontsize="small" if p > 0.05 else "large",
            )
        axin.axis("off")
...
def supp_bench(save_fig=False):
    ...
    grid = plt.GridSpec(
        2, 7, hspace=1, wspace=1, top=0.96, bottom=0.04, left=0.01, right=0.99
    )
    colors_ia = [0.5 * np.ones(3), "k"]
    colors_tab = plt.get_cmap("tab10").colors
    colors = [
    ...
    for j in range(2):
        ...
        pos = ax.get_position().bounds
        ax.set_position(
            [pos[0], pos[1] - 0.1 * (j == 0) - 0.02 * (j == 1), pos[2], pos[3]]
        )
        axin = ax.inset_axes([0.0, 1.02, 1, 0.1])
        ...
        for d in range(len(dsets)):
            ...
            for i in range(len(ips)):
                ...
                if i > 0:
                    ...
                    axin.text(
                        k - i / 2,
                        0.95 + (len(ips) - i) * 0.02 + 0.01 * (p > 0.05),
                        pstr,
                        ha="center",
                        va="center",
                        fontsize="small" if p > 0.05 else "large",
