...
from cellpose import denoise, io, metrics, models, plot, transforms
...
def load_benchmarks(
    ...
    if others:
        dat = np.load(
            root / "noisy_test" / f"test_{noise_type}_n2v.npy",
            allow_pickle=True,
        ).item()
        ...
        dat = np.load(
            root / "noisy_test" / f"test_{noise_type}_n2s.npy",
            allow_pickle=True,
        ).item()
    ...
    if others:
        dat = np.load(
            root / "noisy_test" / f"test_{noise_type}_cp_retrain.npy",
            allow_pickle=True,
        ).item()
...
def fig2(
    ...
    for i, ifr in enumerate(ifrs):
        ...
        ax.set_title(
            "dense expression" if i == 0 else "sparse expression",
            fontsize="medium",
        )
...
def load_benchmarks_specialist(folder, thresholds=np.arange(0.5, 1.05, 0.05)):
    ...
    dat = np.load(
        root / "noisy_test" / f"test_{noise_type}_n2v_specialist.npy",
        allow_pickle=True,
    ).item()
    test_n2v = dat["test_n2v"][:nimg_test]
    masks_n2v = dat["masks_n2v"][:nimg_test]
    ...
    dat = np.load(
        root / "noisy_test" / f"test_{noise_type}_n2s_specialist.npy",
        allow_pickle=True,
    ).item()
    test_n2s = dat["test_n2s"][:nimg_test]
    masks_n2s = dat["masks_n2s"][:nimg_test]
    ...
    for i in range(nd - 1):
        for j in range(3):
            ...
            if img.shape[0] == 1 or img[1].sum() == 0:
                ax.imshow(
                    transforms.normalize99(img[0]) * 1.0,
                    cmap="gray",
                    vmin=0,
                    vmax=1,
                )
