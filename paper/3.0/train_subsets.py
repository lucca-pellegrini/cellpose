...
def main():
    ...
    test_labels = [
        io.imread(root / "test" / f"{i:03d}_img_flows.tif") for i in range(91)
    ]
    ...
    # poisson training
    model_path, train_losses, test_losses = denoise.train(
        model.net,
        ...
        model_name=f"denoise_cyto2_{nsub}_{seed}_{n_epochs}",
    )
...
def save_results(folder, sroot):
    ...
    diam_test = (
        dat["diam_test"][:68] if "diam_test" in dat else 30.0 * np.ones(len(test_noisy))
    )
    ...
    for k, nsub in enumerate(nsubs):
        ...
        for seed in range(5):
            ...
            masks2, flows2, styles2 = seg_model.eval(
                imgs2,
                channels=[1, 0],
                diameter=diam_test,
                channel_axis=-1,
