...
def fig1(imgs_norm, masks_true, dat, timings, save_fig=False):
    ...
    for k, mtype in enumerate(mtypes):
        ...
        vp = ax.violinplot(
            tsec,
            positions=k * np.ones(1),
            bw_method=0.1,
            showextrema=False,
            showmedians=False,
        )  # , quantiles=[[0.25, 0.5, 0.75]])
        ax.plot(
            dx * np.arange(-1, 2, 2) + k,
            np.median(tsec) * np.ones(2),
            color=cols[mtype],
        )
        vp["bodies"][0].set_facecolor(cols[mtype])
        ax.text(
            k + 0.2 if k > 0 else k - 0.1,
            -1,
            titles[mtype].replace(" (", "\n("),
            color=cols[mtype],
            rotation=0,
            va="top",
            ha="center",
    ...
    f1s = np.array(
        [
            [0.8612, 0.8346, 0.7976, 0.7013, 0.4116],
    ...
    for k, mtype in enumerate(mtypes):
        ...
        ax.text(
            0.1,
            0.5 - k * 0.13 if k < 2 else 0.5 - k * 0.13 + 0.05,
            titles[mtype].replace(" (", "\n("),
            color=cols[mtype],
            fontsize="large",
            transform=ax.transAxes,
    ...
    for k, mtype in enumerate(mtypes):
        ax.errorbar(
            thresholds,
            np.median(dat[f"{mtype}_f1"], axis=0),
            dat[f"{mtype}_f1"].std(axis=0) / ((dat[f"{mtype}_f1"].shape[0] - 1) ** 0.5),
            color=cols[mtype],
            lw=2,  # if mtype=="grayscale" else 1,
            ls="--" if mtype == "transformer" else "-",
            zorder=30 if mtype == "maetal" else 0,
        )
    ax.set_ylabel("F1 score")
    ax.set_xlabel("IoU threshold")
    ax.set_yticks(np.arange(0, 1.1, 0.2))
...
def fig2(
    ...
    outlines_gt = [
        utils.outlines_list(masks_true[iex], multiprocessing=False) for iex in ids
    ]
    outlines_cp = [
        utils.outlines_list(dat["default"][iex], multiprocessing=False) for iex in ids
    ]
    outlines_m = [
        utils.outlines_list(dat["mediar"][iex], multiprocessing=False) for iex in ids
    ]
    ...
    grid1 = matplotlib.gridspec.GridSpecFromSubplotSpec(
        1, 4, subplot_spec=ax, wspace=0.15, hspace=0.15
    )
    ax.remove()
    ...
    ax.scatter(
        emb[:-100, 1][irand],
        emb[:-100, 0][irand],
        color=cols[types[:-100]][irand],  # , cmap="tab10",
        s=1,
        alpha=0.5,
        marker="o",
        rasterized=True,
    ...
    im = ax.scatter(
        emb[-100:, 1],
        emb[-100:, 0],
        c=dat["default_f1"][:, 0] - dat["mediar_f1"][:, 0],
        lw=2,
        s=60,
        marker="x",
        alpha=1,
