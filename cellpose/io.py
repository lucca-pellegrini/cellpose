...
def save_rois(masks, file_name, multiprocessing=None):
    ...
    if len(outlines) != len(nonempty_outlines):
        print(
            f"empty outlines found, saving {len(nonempty_outlines)} ImageJ ROIs to .zip archive."
        )
...
def save_masks(
    ...
    if save_mpl and png and MATPLOTLIB and not min(images.shape) > 3:
        ...
        fig.savefig(
            os.path.join(savedir, basename + "_cp_output" + suffix + ".png"),
            dpi=300,
        )
        plt.close(fig)
    ...
    if masks.ndim < 3 and save_outlines:
        ...
        imsave(
            os.path.join(outlinedir, basename + "_outlines" + suffix + ".png"),
            imgout,
        )
