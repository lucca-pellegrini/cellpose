...
def cyto(test_root, save_root, save_figure=False):
    ...
    if save_figure:
        ...
        fig.savefig(
            os.path.join(save_root, "figs/fig_perf2d_cyto.pdf"),
            bbox_inches="tight",
        )
...
def suppfig_metrics(test_root, save_root, save_figure=False):
    ...
    if save_figure:
        os.makedirs(os.path.join(save_root, "figs"), exist_ok=True)
        fig.savefig(
            os.path.join(save_root, "figs/suppfig_metrics.pdf"),
            bbox_inches="tight",
        )
...
def mask_stats(test_root, save_root, save_figure=False):
    ...
    digi = (
        np.digitize(
            np.clip(convexities.copy(), bins[0] + 0.01, bins[-1] - 0.01),
            bins=bins,
        )
        - 1
    )
    ...
    for k in range(3):
        ...
        for l in range(24):
            ...
            ax.imshow(
                test_labels[im][slices[0]] == (imask + 1),
                vmin=0,
                vmax=1,
                cmap="gray_r",
            )
